...
/client/verb/adminhelp(msg as text)
	...
	if(admin_number_active <= 0)
		SSdiscord.post_webhook_event(WEBHOOK_ADMIN_PM_IMPORTANT, list("title"="Help is requested", "message"="Request for Help from **[key_name(src)]**: ```[html_decode(msg)]```\n[admin_number_afk ? "All admins AFK ([admin_number_afk])" : "No admins online"]!!"))
		SSdiscord.send_to_admins("@here Request for Help from [key_name(src)]: [html_decode(msg)] - !![admin_number_afk ? "All admins AFK ([admin_number_afk])" : "No admins online"]!!")
